...
C.Address,
C.City,
E.FirstName + ' ' + LastName as Name_Surname,
S.ShipperName,
S.Phone
FROM
orders AS O
...
--5
SELECT 
Count (P.ProductID) as 'Say',
SUM(P.Price) as 'Umumi Say',
S.SupplierName
FROM 
products AS P
INNER JOIN suppliers as S
...
P.SupplierID=S.SupplierID
WHERE 
S.SupplierName ='Norske Meierier'
...
S.Country,
S.City,
Count(ProductName) as 'Mehsul sayi' 
FROM products as P
INNER JOIN suppliers as S
on 
P.SupplierID=S.SupplierID
...
Select 
P.ProductName,
Count(O.OrderID) as 'Say', 
Count(OD.ProductID) as 'Mehsul sayi',
O.OrderDate
from
products as P
INNER JOIN order_details as OD
...
P.ProductName, 
O.OrderDate 
having COUNT(O.OrderID) >= 2 order by O.OrderDate DESC
